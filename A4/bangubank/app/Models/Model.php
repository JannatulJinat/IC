...
class Model{
    ...
    public function createTable(){
        ...
        foreach($sqlFiles as $sqlFile){
            $sqlFileContent = trim(file_get_contents($sqlFile));
    ...
    public function setupDatabaseConnection(){
        ...
        try {
            $this->pdoConnection = new PDO("mysql:host={$config['host']}", $config['username'], $config['password']);
            $createDB = "CREATE DATABASE IF NOT EXISTS {$config['dbname']}";
            $this->pdoConnection->exec($createDB);
